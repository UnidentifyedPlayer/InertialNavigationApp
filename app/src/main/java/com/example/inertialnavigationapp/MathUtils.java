...
public class MathUtils {
    ...
    public static void rotationMatrixFromRotationVector(float[] vector, float[] rotationMatrix){
        ...
        for(int k = 0; k<3; k++){
            for(int j = 0; j<3; j++){
                if(j==k){
                    rotationMatrix[j+k*3] = (float)(2*Math.pow(vector[1+j],2)+2*Math.pow(vector[0],2)-1);
                }
                else{
                    l = 3 - j - k;//index of value in vector variable that is not j or k
                    rotationMatrix[j+k*3] = 2*(vector[1+j]*vector[1+k]) - 2*LeviCivita(j,k,l)*vector[1+l]*vector[0];
                }
            }
        }
    }
    ...
    public static float[] constructRotationVector(float angle, float[] axisVector){
        ...
        return new float[] { rotationCos, axisVector[0]*magnitude, axisVector[1]*magnitude,
                axisVector[2]*magnitude};
    }
